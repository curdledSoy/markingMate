<template>
  <div>
    ...
    <h1 class="font-thin text-sm">{{ rubric.semester }}, {{ rubric.year }}</h1>
    <div class="space-x-3">
      <template
        v-if="
          rubricState['isEditing'] == false && rubricState['isMarking'] == false
        "
      >
        <BaseButton>Export</BaseButton>
        <BaseButton color="tertiary" @click="changeRubricState('isEditing')"
          >Edit</BaseButton
        >
      </template>
      <template v-if="rubricState['isEditing'] || rubricState['isMarking']">
        <BaseButton color="tertiary" @click="changeRubricState('isSaving')"
          >Save</BaseButton
        >
        <BaseButton color="secondary" @click="changeRubricState('isCancel')"
          >Cancel</BaseButton
        >
...
let { undo, redo, canUndo, canRedo, ...history } = useRefHistory(rubric, {
  deep: true,
});
...
let addGrade = () => {
  id++
  let grade = { id: id, name: "New Grade" };
  rubric.value.grades.push(grade);
  rubric.value.objectives.forEach((objective) => {
    rubric.value.outcomes[objective.id][grade.id] = {
      outcome: "New Outcome",
      description: "New Outcome for New Grade",
    };
  });
};
